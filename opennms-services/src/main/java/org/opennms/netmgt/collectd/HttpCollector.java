...
public class HttpCollector implements ServiceCollector {
    ...
    private static void processResponse(final Locale responseLocale, final String responseBodyAsString, final HttpCollectorAgent collectorAgent, final CollectionSetBuilder collectionSetBuilder) {
        ...
        int numberOfButes = 0;
        int flags = 0;
        if (collectorAgent.getUriDef().getUrl().getCanonicalEquivalence()) {
            flags |= Pattern.CANON_EQ;
        }
        if (collectorAgent.getUriDef().getUrl().getCaseInsensitive()) {
            flags |= Pattern.CASE_INSENSITIVE;
        }
        if (collectorAgent.getUriDef().getUrl().getComments()) {
            flags |= Pattern.COMMENTS;
        }
        if (collectorAgent.getUriDef().getUrl().getDotall()) {
            flags |= Pattern.DOTALL;
        }
        if (collectorAgent.getUriDef().getUrl().getLiteral()) {
            flags |= Pattern.LITERAL;
        }
        if (collectorAgent.getUriDef().getUrl().getMultiline()) {
            flags |= Pattern.MULTILINE;
        }
        if (collectorAgent.getUriDef().getUrl().getUnicodeCase()) {
            flags |= Pattern.UNICODE_CASE;
        }
        if (collectorAgent.getUriDef().getUrl().getUnixLines()) {
        ...
        if (matches) {
            ...
            final List<Attrib> attribDefs = collectorAgent.getUriDef().getAttributes().getAttribCollection();
            ...
            for (final Attrib attribDef : attribDefs) {
                final AttributeType attributeType = AttributeType.parse(attribDef.getType());
                if (attributeType == null) {
                    LOG.error("Unsupported attribute type: {}", attribDef.getType());
                    continue;
                }
                ...
                if (attributeType.isNumeric()) {
                    ...
                    collectionSetBuilder.withNumericAttribute(resource, collectorAgent.getUriDef().getName(), attribDef.getAlias(), num, attributeType);
                    numberOfButes++;
                } else {
    ...
    private static List<NameValuePair> buildRequestParameters(final HttpCollectorAgent collectorAgent) {
        ...
        List<Parameter> parameters = collectorAgent.getUriDef().getUrl().getParameters().getParameterCollection();
