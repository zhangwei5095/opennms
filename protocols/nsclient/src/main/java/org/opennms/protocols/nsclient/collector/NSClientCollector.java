                    ...
                    for (Attrib attrib : wpm.getAttrib()) {
                        ...
                        if (result != null) {
                            if (result.getResultCode() != NsclientPacket.RES_STATE_OK) {
                                ...
                                if (type == null) {
                                    LOG.warn("Unsupported type '{}' for attribute named '{}' in group named '{}'. Values for this attribute will not be persisted.",
                                            attrib.getType(), attrib.getName(), wpm.getName());
                                    continue;
                                }

                                // Only numeric data comes back from NSClient in data collection
                                builder.withNumericAttribute(nodeResource, wpm.getName(), attrib.getAlias(), NumericAttributeUtils.parseNumericValue(result.getResponse()), type);
                            }
                        }
                    }
                    builder.withStatus(CollectionStatus.SUCCEEDED);
