...
public class WmiCollector implements ServiceCollector {
    ...
    private Map<String, AttributeType> m_attribTypeList = new HashMap<>();
                    ...
                    if (wOS != null) {
                        ...
                        for (int i = 0; i < wOS.count(); i++) {
                            ...
                            for (final Attrib attrib : wpm.getAttrib()) {
                                ...
                                final AttributeType type = m_attribTypeList.get(attrib.getName());
                                if (type == null) {
                                    LOG.warn("Skipping attributed named '{}' with unknown attribute type.", attrib.getName());
                                    continue;
                                }
    ...
    private void loadAttributeTypeList(final WmiCollection collection) {
        for (final Wpm wpm : collection.getWpms().getWpm()) {
            for (final Attrib attrib : wpm.getAttrib()) {
                final AttributeType type = AttributeType.parse(attrib.getType());
                if (type == null) {
                    LOG.warn("Unsupported type '{}' for attribute named '{}' in group '{}'. Attributes for this column will not be persisted.",
                            attrib.getType(), attrib.getName(), wpm.getName());
                    continue;
                }
                m_attribTypeList.put(attrib.getName(), type);
            }
        }
    }
